...
\subsubsection{Arquitectura Final}
...
\subsubsection{\texttt{void qpx\_VHC\_transfer(VEHICLE* vehicle, LINK* link1, LINK* link2)}}

Este método es ejecutado por Paramics cada vez que un vehículo pasa de una calle a otra. También se utiliza en el \emph{plugin} para notificar a \texttt{VehicleManager} para que actualice su estado interno.
...
En específico, se utilizan las siguientes variables para almacenar información sobre el estado de la simulación en todo instante:

\begin{description}[]
    \item[\texttt{std::unordered\_map<int, VEHICLE*>\@ vehicles\_in\_sim}] \emph{Hashmap} que almacena el ID y un puntero a cada vehículo presente en la simulación. Se utiliza ya que Paramics no provee un método directo para obtener un puntero a un vehículo dada su ID, sino que es necesario buscarlo en la red. Este método elimina esa búsqueda y facilita además el conteo de vehículos en la simulación (basta con obtener la cantidad de pares \texttt{\{llave, valor\}} en el \emph{hashmap}). Se actualiza dinámicamente cada vez que ingresa un vehículo nuevo a la red, a través del llamado al método \texttt{vehicleDepart()} del presente módulo desde \texttt{plugin.c}.
\end{description}
